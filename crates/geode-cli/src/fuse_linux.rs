//! Linux FUSE session (08-mount 2; SPEC-v030 G1).
//!
//! `fuser` lives only in this file, behind feature `fuse` and
//! `cfg(target_os = "linux")`. `geode-grotto` never depends on it. A core
//! build and the default `cargo test` do not compile this module, so a
//! missing `/dev/fuse` cannot fail Darwin or GHA.
//!
//! The filesystem is a thin adapter over [`geode_grotto::mount_session::MountSession`].
//! Foreground `mount2` blocks until `fusermount3 -u`. `destroy` persists the
//! manifest through `MountSession::unmount`.
...
use super::super::{load_isk, load_vault};
...
/// Foreground mount. Blocks until `geode unmount` / `fusermount3 -u`.
...
/// `geode unmount` (08-mount 6): fusermount3, not a kernel-free stub.
